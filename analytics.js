(function() {
	...
	var tracker = {
		...
		clientConfig : {
			...
			serverUrl : "http://117.184.229.61:8082/cntv.gif",
		...
		onPageView : function() {
			...
			if (this.preCallApi()) {
				...
				pageviewEvent[this.columns.appId] = "2"; //
				pageviewEvent[this.columns.clientType] = "3"; //
				pageviewEvent[this.columns.uuid] = this.getUuid(); //
                ...
                pageviewEvent[this.columns.sessionId] = this.getSid(); // 设置sid
                pageviewEvent[this.columns.visitTime] = new Date().getTime(); // 设置客户端时间
                pageviewEvent[this.columns.ua] = window.navigator.userAgent; // 设置浏览器类型
                pageviewEvent[this.columns.resolution] = screen.width + "*" + screen.height; // 设置浏览器分辨率
